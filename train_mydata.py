...
def main():
    ...
    global val_loss, train_loss, val_acc, train_acc
    ...
    # create model
    if args.arch == "resnet":
        model = ResidualNet( 'CIFAR10', args.depth, 8, args.att_type )
    ...
    # Data loading code
    traindir = './FERPlus/data_new/FER_Train' #os.path.join(args.data, 'train')
    valdir = './FERPlus/data_new/FER_Valid'#os.path.join(args.data, 'val')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1, loss_temp = validate(val_loader, model, criterion, epoch)
        
        val_acc[epoch] = prec1
        val_loss[epoch] = loss_temp
    ...
    np.save(args.prefix + '_val_loss', val_loss)
    np.save(args.prefix + '_val_acc', val_acc)
    np.save(args.prefix + '_train_loss', train_loss)
    np.save(args.prefix +  '_train_acc', train_acc)
